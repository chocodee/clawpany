...
import { runOpenClaw } from "../openclaw-worker/index.js";
...
async function runAutogen(prompt) {
  const { spawn } = await import("node:child_process");
  return new Promise((resolve, reject) => {
    const child = spawn("python", ["workers/autogen-worker/runner.py", prompt], {
...
async function workOnTask(task) {
  ...
  try {
    if (task.runtime === "openclaw") {
      summary = await runOpenClaw(prompt);
    } else if (task.runtime === "autogen") {
      summary = await runAutogen(prompt);
